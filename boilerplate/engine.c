...
static int child_fn(void *arg)
{
    ...
    if (dup2(cfg->log_fd, STDOUT_FILENO) < 0) {
        perror("dup2(stdout)");
        return 1;
    }

    if (dup2(cfg->log_fd, STDERR_FILENO) < 0) {
        perror("dup2(stderr)");
        return 1;
    }
                            ...
                            size_t errbuf_len)
{
...

    {
        ...
        shell_len = snprintf(shell_path, sizeof(shell_path), "%s/bin/sh", req->rootfs);
        if (shell_len < 0 || (size_t)shell_len >= sizeof(shell_path)) {
            snprintf(errbuf, errbuf_len, "rootfs path is too long");
            return -1;
        }

        snprintf(target_path, sizeof(target_path), "%s", shell_path);
        if (lstat(shell_path, &st) == 0 && S_ISLNK(st.st_mode)) {
            ssize_t link_len = readlink(shell_path, link_target, sizeof(link_target) - 1);
            if (link_len < 0) {
                snprintf(errbuf, errbuf_len, "failed to read /bin/sh symlink");
                return -1;
            }

            link_target[link_len] = '\0';
            if (link_target[0] == '/') {
                shell_len = snprintf(target_path,
                                     sizeof(target_path),
                                     "%s%s",
                                     req->rootfs,
                                     link_target);
            ...
            if (shell_len < 0 || (size_t)shell_len >= sizeof(target_path)) {
                snprintf(errbuf, errbuf_len, "/bin/sh symlink target is too long");
                return -1;
            }
        }
        ...
        if (access(target_path, X_OK) != 0) {
            snprintf(errbuf,
                     errbuf_len,
                     "rootfs is missing executable /bin/sh");
            return -1;
        }
    }
    ...
    log_fd = open(rec->log_path, O_CREAT | O_WRONLY | O_TRUNC, 0644);
    if (log_fd < 0) {
